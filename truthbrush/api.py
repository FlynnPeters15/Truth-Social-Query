...
try:
    ...
    CurlError = curl_cffi.curl.CurlError
    RequestError = http_requests.RequestsError
    SUPPORTS_IMPERSONATE = True
except ModuleNotFoundError:
    import requests as http_requests
    from requests import RequestException as RequestError

    CurlError = RequestError
...
class Api:
    ...
    def __check_login(self):
        ...
        if self.auth_id is None:
            ...
            token_preview = (
                f"{self.auth_id[:6]}...{self.auth_id[-4:]}"
                if isinstance(self.auth_id, str) and len(self.auth_id) > 12
                else "<hidden>"
            )
            logger.warning(f"Using token {token_preview}")
    ...
    def get_auth_id(self, username: str, password: str) -> str:
        ...
        try:
            ...
            sess_req = http_requests.request(
                "POST",
                url,
                json=payload,
                headers={
